...
impl AppConfig {
    pub fn load() -> Result<Self, Box<dyn Error>> {
        ...
        let q_instrument = Self::validate_input(&env::var("QUERY_INSTRUMENT").unwrap_or_else(|_| "EURUSD".to_string()), "Instrument")?;
        let q_timeframe = Self::validate_input(&env::var("QUERY_TIMEFRAME").unwrap_or_else(|_| "m1".to_string()), "Timeframe")?;
        let q_start = Self::validate_input(&env::var("QUERY_START_DATE").expect("START DATE not set"), "Start Date")?;
        let q_end = Self::validate_input(&env::var("QUERY_END_DATE").expect("END DATE not set"), "End Date")?;
