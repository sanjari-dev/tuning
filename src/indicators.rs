...
pub fn calculate_vwma(prices: &[f64], volumes: &[f64], period: usize) -> Vec<Option<f64>> {
    let len = prices.len();
    let mut result = vec![None; len];
    if period == 0 || len < period || len != volumes.len() {
        return result;
    }
    ...
    if sum_v != 0.0 {
        result[period - 1] = Some(sum_pv / sum_v);
    }
...
pub fn calculate_dema(data: &[f64], period: usize) -> Vec<Option<f64>> {
    ...
    for i in valid_start..len {
        if let Some(v) = ema1[i] {
            ema1_raw[i] = v;
        }
    }

    let mut result = vec![None; len];
    if valid_start + period > len {
        return result;
    }
...
pub fn calculate_tema(data: &[f64], period: usize) -> Vec<Option<f64>> {
    ...
    let calc_next_ema = |prev_series: &[Option<f64>]| -> Vec<Option<f64>> {
        ...
        let valid_data: Vec<f64> = prev_series
            .iter()
            .skip(start_idx)
        ...
        if next_ema_full.len() < len {
            next_ema_full.resize(len, None);
        }
        next_ema_full
    };
    ...
    for i in (period - 1)..len {
        if let Some(ma) = sma_tp[i] {
            ...
            if mean_dev == 0.0 {
                ...
                result[i] = Some((tp[i] - ma) / (0.015 * mean_dev));
            }
        }
    }
    result
...
pub fn calculate_roc(data: &[f64], period: usize) -> Vec<Option<f64>> {
    ...
    if len > period {
        for i in period..len {
            ...
            if prev != 0.0 {
                result[i] = Some(((data[i] - prev) / prev) * 100.0);
            }
        }
    }
    result
...
pub fn calculate_adx_full(
    ...
    if period == 0 || len < period * 2 {
        return (adx_res, pdi_res, mdi_res);
    }
...
pub fn calculate_envelope_full(
    data: &[f64],
    period: usize,
) -> (Vec<Option<f64>>, Vec<Option<f64>>, Vec<Option<f64>>) {
...
pub fn calculate_cmo(data: &[f64], period: usize) -> Vec<Option<f64>> {
    ...
    for i in 1..=period {
        if data[i] > data[i - 1] {
            up_sum += data[i] - data[i - 1];
        } else {
            down_sum += data[i - 1] - data[i];
        }
    }

    let calc = |u, d| (u - d) / (u + d) * 100.0;
    if up_sum + down_sum != 0.0 {
        result[period] = Some(calc(up_sum, down_sum));
    }
    ...
    for i in (period + 1)..len {
        ...
        if up_sum + down_sum != 0.0 {
            result[i] = Some(calc(up_sum, down_sum));
        }
    }
    result
}
...
impl LocalBuilder {
    ...
    fn add_arc(&mut self, name: &str, arr: Arc<dyn Array>) {
        ...
        if self.offset < arr.len() {
            self.cols
                .push(arr.slice(self.offset, arr.len() - self.offset));
        } else {
            self.cols
                .push(Arc::new(Float64Array::from(Vec::<Option<f64>>::new())));
        }
    }
    ...
    let results: Vec<(Vec<Field>, Vec<Arc<dyn Array>>)> = (min_period..=max_period)
        ...
        .map(|period| {
            ...
            if let Some(arr) = gpu_smas.as_ref().and_then(|m| m.get(&period)) {
                ...
                builder.add_vec(
                    &format!("sma_{}", period),
                    calculate_sma(&close_ref, period),
                );
            }

            builder.add_vec(
                &format!("ema_{}", period),
                calculate_ema(&close_ref, period),
            );
            ...
            if let Some(arr) = gpu_wmas.as_ref().and_then(|m| m.get(&period)) {
                ...
                builder.add_vec(
                    &format!("wma_{}", period),
                    calculate_wma(&close_ref, period),
                );
            }

            builder.add_vec(
                &format!("vwma_{}", period),
                calculate_vwma(&close_ref, &vol_ref, period),
            );
            builder.add_vec(
                &format!("hma_{}", period),
                calculate_hma(&close_ref, period),
            );
            builder.add_vec(
                &format!("smma_{}", period),
                calculate_smma(&close_ref, period),
            );
            builder.add_vec(
                &format!("dema_{}", period),
                calculate_dema(&close_ref, period),
            );
            builder.add_vec(
                &format!("tema_{}", period),
                calculate_tema(&close_ref, period),
            );
            builder.add_vec(
                &format!("rsi_{}", period),
                calculate_rsi(&close_ref, period),
            );
            builder.add_vec(
                &format!("cci_{}", period),
                calculate_cci(&high_ref, &low_ref, &close_ref, period),
            );
            builder.add_vec(
                &format!("mom_{}", period),
                calculate_momentum(&close_ref, period),
            );
            builder.add_vec(
                &format!("roc_{}", period),
                calculate_roc(&close_ref, period),
            );
            builder.add_vec(
                &format!("wpr_{}", period),
                calculate_williams_r(&high_ref, &low_ref, &close_ref, period),
            );
            builder.add_vec(
                &format!("atr_{}", period),
                calculate_atr(&high_ref, &low_ref, &close_ref, period),
            );
            ...
            builder.add_vec(
                &format!("trix_{}", period),
                calculate_trix(&close_ref, period),
            );
            builder.add_vec(
                &format!("sm_rsi_{}", period),
                calculate_rsi(&close_ref, period),
            );
            ...
            builder.add_vec(
                &format!("fi_{}", period),
                calculate_force_index(&close_ref, &vol_ref, period),
            );
            builder.add_vec(
                &format!("cmf_{}", period),
                calculate_cmf(&high_ref, &low_ref, &close_ref, &vol_ref, period),
            );
            builder.add_vec(
                &format!("mfi_{}", period),
                calculate_mfi(&high_ref, &low_ref, &close_ref, &vol_ref, period),
            );
            ...
            builder.add_vec(
                &format!("aroon_osc_{}", period),
                calculate_aroon_osc(&ar_u, &ar_d),
            );
            builder.add_vec(
                &format!("cmo_{}", period),
                calculate_cmo(&close_ref, period),
            );
            ...
            if let (Some(s_map), Some(v_map)) = (&gpu_std_map, &gpu_var_map) {
                if let (Some(d_s), Some(d_v)) = (s_map.get(&period), v_map.get(&period)) {
                    ...
                    builder.add_vec(
                        &format!("std_dev_{}", period),
                        calculate_std_dev(&close_ref, period),
                    );
                    builder.add_vec(
                        &format!("variance_{}", period),
                        calculate_variance(&close_ref, period),
                    );
                }
            } else {
                builder.add_vec(
            ...
            builder.add_vec(
                &format!("median_{}", period),
                calculate_median(&close_ref, period),
            );
            ...
            builder.add_vec(
                &format!("obv_sma_{}", period),
                calculate_sma(&obv_ref, period),
            );
