...
impl Phase1Cleaner {
    pub fn new(schema: SchemaRef, start_time: Instant) -> Self {
        ...
        println!(
            "[{:>8.2?}] 🧹 [Phase 1] Initialized cleaner structure for {} columns",
            start_time.elapsed(),
            num_cols
        );
        Phase1Cleaner {
    ...
    pub fn check_batch(&mut self, batch: &RecordBatch) {
        ...
        self.log(
            "🔍",
            format!(
                "Processing Batch #{}: Scanning {} columns (Parallel)",
                self.batch_counter, num_cols
            ),
        );
        let batch_stats: Vec<Option<(f64, f64)>> = (0..num_cols)
            .into_par_iter()
            .map(|i| {
        ...
        let new_groups: Vec<Vec<usize>> = current_groups
            .into_par_iter()
            .flat_map(|group| {
                if group.len() <= 1 {
                    return vec![group];
        ...
        if new_groups.len() > initial_groups_count {
            self.log(
                "⚡",
                format!(
                    "Duplicate groups refined: {} groups -> {} unique groups found in this batch",
                    initial_groups_count,
                    new_groups.len()
                ),
            );
        }
    ...
    pub fn get_results(&self) -> (Vec<String>, Vec<String>) {
        self.log(
            "🧠",
            "Finalizing results. Analyzing variance statistics and duplicate groups",
        );
            ...
            if dtype == &DataType::Float64 {
                if let Some((min, max)) = self.min_max[i] {
                    if (max - min).abs() < 1e-9 {
                        ...
                        drop_reasons.insert(
                            fields[i].name().clone(),
                            "Zero Variance (Constant Value)".to_string(),
                        );
                    }
                } else {
                    zero_variance_indices.push(i);
        ...
        self.log(
            "📉",
            format!(
                "Found {} features with Zero Variance or All-Nulls",
                zero_variance_indices.len()
            ),
        );
        ...
        self.log(
            "👯",
            format!(
                "Identified {} redundant features (Duplicates)",
        ...
        let keep_names: Vec<String> = keep_indices
            .iter()
